...
public class SearchServiceImpl implements ISearchService {
    ...
    private void handleMessage(String content) {
        try {
            ...
            switch (message.getOperation()) {
                case HouseIndexMessage.INDEX://构建索引的消息
                    this.createOrUpdateIndex(message);
                    break;
                case HouseIndexMessage.REMOVE://删除索引的消息
    ...
    private void index(Long houseId, int retry) {
        if (retry > HouseIndexMessage.MAX_RETRY) {//重试次数过多，直接返回
...
//        );
        ...
        SearchRequestBuilder requestBuilder = this.esClient.prepareSearch(INDEX_NAME)
                ...
                .setQuery(boolQuery)
                .addSort(
                        HouseSort.getSortKey(rentSearch.getOrderBy()),//排序的字段
                        SortOrder.fromString(rentSearch.getOrderDirection())//排序的方向
                )
    ...
    public ServiceResult<Long> aggregateDistrictHouse(String cityEnName, String regionEnName, String district) {
        ...
        SearchRequestBuilder requestBuilder = this.esClient.prepareSearch(INDEX_NAME)
                ...
                .setQuery(boolQuery)
                .addAggregation(
                        //对HouseIndexKey.DISTRICT字段聚合，聚合后的名字为HouseIndexKey.AGG_DISTRICT
                        AggregationBuilders.terms(HouseIndexKey.AGG_DISTRICT).field(HouseIndexKey.DISTRICT))
                .setSize(0);//不需要查询数据，只需要聚合
